...
async function editRole(selected, item) {
  ...
  await getUsers();
  newInfo.value = ""; // empty newInfo for later use
}
        ...
        <v-col cols="8">
          <div id="resumes">
            <v-card variant="elevated" flex="no-wrap">
              <v-row>
                <v-card-title class="text-h4 font-weight-bold">Resumes</v-card-title>
              </v-row>
            ...
            <v-card variant="elevated">
                      ...
                      <v-menu transition="scale-transition">
                        ...
                        <v-list>
                          <v-list-item v-for="(items, i) in roles" :key="i" :value="i">
                            <v-list-item-title @click="editRole(i, item)">{{ items }}</v-list-item-title>
                          </v-list-item>
                        </v-list>
                      </v-menu>
                    </td>
              ...
              <v-dialog :model-value="isEditName" width="800">
                <v-card class="rounded-lg elevation-5">
                  ...
                  <v-card-actions>
                    <v-spacer></v-spacer>
                    <v-btn variant="flat" color="secondary" @click="isEditName ? closeEdit() : false">Close</v-btn>
                    <v-btn variant="flat" color="primary" @click="saveUser()">{{ isEditName ? "Update User Profile" :
                      "" }}</v-btn>
                  </v-card-actions>
                </v-card>
              </v-dialog>
